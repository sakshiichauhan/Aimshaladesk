...
export function Bugs() {
  ...
  const handleViewBug = (bugId: number) => {
    navigate(`/desk/relation/cases/bugs/view`, {
      state: { id: bugId }
    });
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <StatsCards />
      <Buttonbar />
      <BugTable onViewBug={handleViewBug} />
    </div>
  );
}
...
function Buttonbar() {
  const navigate = useNavigate();
  const onNewBug = () => {
    navigate('/desk/relation/cases/bugs/addNewBug', {
      state: { text: 'Bug' }
    });
  };
  return (
    <div className="flex justify-between px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      <Button variant="brand" size="new" onClick={onNewBug}>
...
function BugTable({ onViewBug }: BugTableProps) {
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="actionIcon" size="actionIcon" onClick={() => onViewBug(bugs.id)}>
