...
import { MapListingsReview, coachesList } from "@/data/Data";
import { cn } from "@/lib/utils";

export default function MapListingTable() {
    const [usersData, setUsersData] = useState(MapListingsReview);
    ...
    const handleRowClick = (user: (typeof MapListingsReview)[0]) => {
            ...
            <Table className="w-full caption-top border-collapse overflow-y-visible">
              <TableHeader className="bg-[var(--faded)] hover:bg-[var(--faded)] dark:bg-[var(--faded)] opacity-100">
                <TableRow>
                  ...
                  <TableHead
                    onClick={() => requestSort("Listing")}
                    className="cursor-pointer text-[var(--text)] text-low"
                  >
                    Listing {" "}
                    {sortConfig?.key === "Listing" &&
                      (sortConfig.direction === "ascending" ? "↑" : "↓")}
                  </TableHead>
                  <TableHead
                  ...
                  <TableHead
                    onClick={() => requestSort("LinkedOrg")}
                    className="cursor-pointer text-[var(--text)]"
                  >
                    Linked Org{" "}
                    {sortConfig?.key === "LinkedOrg" &&
                      (sortConfig.direction === "ascending" ? "↑" : "↓")}
                  </TableHead>
                  <TableHead
                    onClick={() => requestSort("MapPreview")}
                    className="cursor-pointer text-[var(--text)]"
                  >
                    Map Preview{" "}
                    {sortConfig?.key === "MapPreview" &&
                      (sortConfig.direction === "ascending" ? "↑" : "↓")}
                  </TableHead>
                  <TableHead
              ...
              <TableBody className="overflow-visible relative z-0">
                {currentRecords.map((user) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="text-low">{user.Listing}</div>
                      <div className="text-xs text-[var(--text)]">
                        {user.id}
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="standard">{user.Address}</Badge>
                    </TableCell>
                    
                    <TableCell>{user.submittedOn}</TableCell>
                    <TableCell>{user.MapPreview}</TableCell>
                    <TableCell>
