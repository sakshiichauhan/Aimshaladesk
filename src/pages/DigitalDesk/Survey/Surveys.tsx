...
// TypeScript interface for survey data
...
function Bar() {
  ...
  return (
    <div className="flex justify-between items-center px-4 py-3 bg-[var(--background)] rounded-sm gap-4 border flex-wrap shadow-none">
      ...
      <Button variant="outline" size="new"  onClick={() => navigate("manage-question")}>
          <BadgeQuestionMark className="h-3 w-3" />
          <span className=""> Manage Surveys</span>
        </Button>
        <DatePickerWithRange />
...
function TableSection() {
  ...
  const [selectedStack, setSelectedStack] = useState<SurveyData[]>(
  ...
  if (sortConfig !== null) {
    sortedData.sort((a, b) => {
      ...
      if (sortConfig.key === "questions") {
        // Handle questions array for sorting (convert to string for comparison)
        aValue = Array.isArray(aValue) ? aValue.join(", ") : aValue;
        bValue = Array.isArray(bValue) ? bValue.join(", ") : bValue;
      }
  ...
  const handleRowClick = (user: SurveyData) => {
            ...
            <TableBody className="overflow-visible relative z-0">
              {currentRecords.map((user) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="text-low">{Array.isArray(user.for) ? user.for.join(", ") : user.for}</div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="standard">{Array.isArray(user.questions) ? user.questions.join(", ") : user.questions}</Badge>
                  </TableCell>
